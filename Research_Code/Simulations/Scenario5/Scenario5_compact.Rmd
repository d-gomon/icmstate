...
method_names <- c("Multinomial EM", "Poisson EM", "Homogeneous")
names(method_names) <- c("multinomial", "poisson", "msm")
method_colors <- c("#1B9E77", "#7570B3", "#D95F02")
names(method_colors) <- c("multinomial", "poisson", "msm")
...
for(i in 1:length(n)){
  ...
  colnames(stat_multiple)[5] <- "Method"
  plot_all_stat_pt <- ggplot(data = stat_multiple, aes(x = time, y = stats, group = interaction(type, as.factor(Method)), col = as.factor(Method), linetype = as.factor(type))) + geom_line(lwd = 1.2) + facet_grid(trans~., scales = "free", labeller = as_labeller(trans_names)) + xlab("Time (Years)") + ylab("Value") + labs(title = "Transition Probabilities", subtitle = paste0("Scenario ", scenario,  ", n = ", n[i]), color = "Method", linetype = "Measure") + scale_color_manual(labels = method_names, values = method_colors) #+ scale_color_brewer(palette="Dark2", labels = method_names) 
