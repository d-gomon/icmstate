...
toothMSMHom_simple_fixed <- msm::msm(state ~ time, data = gd_msm_simple, subject = id, qmatrix = qmat_try_simple, gen.inits = FALSE, pci = c(6, 8, 10),  control=list(fnscale=12000, maxit = 1000), fixedpars = c(3, 4, 5))
```
...
fit1_plot_new <- ggplot() +  
      ...
      coord_cartesian(ylim=c(0, 4), xlim = c(6, 12.5)) +
...
fit2_plot_new <- ggplot() +  
      ...
      xlab("Time") + 
      ylab("Cumulative Hazard") +
      coord_cartesian(ylim=c(0, 0.75), xlim = c(6, 12.5)) +
      labs(colour='Transition', linetype = "Method") +
