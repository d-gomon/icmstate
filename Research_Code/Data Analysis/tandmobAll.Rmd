...
The following 9 transitions are possible:
...
tmat <- transMat(list(c(2), c(3, 4), c(5), c(5, 6, 7), c(6), c(), c(6)), 
...
```{r}
library(msm)
statetable <- msm::statetable.msm(state, id, gd)
...
asd <- survfit(Surv(left, right, event, type = "interval2") ~ 1, data = visdat)
plot(asd)
```
...
#Can't get MSM to work. Somehow crudeinits does not assign any mass to 1->2 transition.
#toothMSM <- msm::msm(state ~ time, subject = id, data = gd2, qmatrix = qmatrix, gen.inits = TRUE)
...
Determine where to put the cut-points:
```{r}
ptf <- probtrans(toothMSMmult$A, predt = 0, direction = "forward", variance = FALSE)
#We want to find the first non zero and maximum of P_{D->P46}(0, t) over t, so P_{12}(0, t)
#[, 3] because first colum is time
ptfs <- ptf[[1]][, 3]
max_times <- ptf[[1]][which.max(ptfs), 1]
first_nonzero <- ptf[[1]][which.max(ptfs != 0), 1]
```
...
# Try with simpler data to fit Homogeneous model
...
qmat_try_simple[5, 5] <- -0.1

toothMSMHom_simple <- msm::msm(state ~ time, data = gd_msm_simple, subject = id, qmatrix = qmat_try_simple, gen.inits = FALSE, pci = c(first_nonzero, max_times), control=list(fnscale=12000, maxit = 1000))
```
...
fit2_plot_new <- ggplot() +  
      ...
      xlab("Time") + 
      ylab("Cumulative Hazard") +
      coord_cartesian(ylim=c(0, 0.75), xlim = c(0, 12)) +
      labs(colour='Transition', linetype = "Method") +
...
#qwe <- ggarrange(a, fit1_plot, fit2_plot, ncol = 3, labels = "AUTO", legend = "bottom")
